...
public class ContentCategoryServiceImpl extends MyServiceImpl<ContentCategory> implements ContentCategoryService {
    ...
    public List<ContentCategory> queryContentCategoryValidList() {
        List<ContentCategory> contentCategoryList = redisTemplate.opsForList().range(CuttlefishRedisKeyConstant.REDIS_KEY_CONTENT_CATEGORY_LIST, 0, -1);
        if (contentCategoryList.size() == 0){
            ...
            redisTemplate.opsForList().leftPushAll(CuttlefishRedisKeyConstant.REDIS_KEY_CONTENT_CATEGORY_LIST, contentCategoryList);
        }
